...
TaskManager::TaskManager(double _totalNodes, int _maxQueueSize)
    : freeNodes(_totalNodes), totalNodes(_totalNodes), completedTasks(0), busyTime(0),
    currentTime(0), maxQueueSize(_maxQueueSize) {}

void TaskManager::AddTask(const Task& task)
{
    if (taskQueue.GetSize() + activeTasks.size() < maxQueueSize && task.GetNode() <= freeNodes)
    {
        taskQueue.Put(task);
